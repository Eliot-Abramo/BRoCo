...
{
  DummySystem_DummyPacket packet;
  packet.data = msg->data;
  sender->send<DummySystem_DummyPacket>(&packet);
  std::cout << "Callback happened with packet value : " << packet.data << std::endl;
}

void handle_dummy(uint8_t sender_id, DummySystem_DummyPacket* packet, void* ros_publisher)
{
  ...
  auto publisher = reinterpret_cast<rclcpp::Publisher<std_msgs::msg::Int32>::SharedPtr*>(ros_publisher);
  (*publisher)->publish(msg);
}
